...
interface ReportsSectionProps {
  ...
  totalAssets: number;
  setIsManageTypesDialogOpen: (open: boolean) => void;
}
      ...
      <Card className="backdrop-blur-xl bg-white/60 border border-white/20 shadow-xl shadow-black/5 rounded-2xl w-full">
        <CardHeader className="pb-8 flex flex-row items-center justify-between">
          ...
          <Button
            ...
            variant="outline"
            className="backdrop-blur-sm bg-white/50 border-white/30 hover:bg-white/70 rounded-xl font-semibold"
          >
            管理类别
          </Button>
        </CardHeader>
            ...
            {categoryStats.map((item) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium">总数量</p>
                </div>
                <div className="mt-4">
                ...
                overdueAssets.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Badge variant="destructive">逾期 {daysOverdue} 天</Badge>
                    </div>
                  );
                })
              ) : (
                <p className="text-center text-gray-500 py-4">暂无逾期资产。</p>
              )}
            </div>
          </CardContent>
        </Card>
